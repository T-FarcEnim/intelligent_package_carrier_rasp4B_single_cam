...
"""
realtime_qr_preview_no_restrictions.py
实时预览二维码识别情况（无限制版）

功能：
1. 实时显示摄像头画面
2. 实时检测并标记任何二维码（无白名单和位数限制）
3. 显示检测结果和距离信息
4. 显示帧率和状态信息
...
class QRCodeRealtimePreviewNoRestrictions:
    def __init__(self, camera_config="config/camera_intrinsics.json", 
        ...
        # 初始化二维码检测器（无限制版）
        self.detector = QRDetector(camera_config, qr_config, debug_mode=False)
        ...
        print("QR Code Real-time Preview (No Restrictions)")
        print("=" * 60)
        print("Now detects ANY QR code (no whitelist or digit limit)")
        print("Controls:")
        print("  'q' or ESC: Exit")
    ...
    def draw_info_panel(self, frame, result):
        ...
        # 添加标题
        cv2.putText(info_panel, "QR Code Detection (No Restrictions)", (10, 30), 
                   font, 0.6, self.COLOR_CYAN, 1)
        ...
        if not result["lost"]:
            # 显示二维码信息
            qr_text = str(result['data'])[:20]  # 限制显示长度
            if len(str(result['data'])) > 20:
                qr_text += "..."
                
            cv2.putText(info_panel, f"QR Data: {qr_text}", (10, y_offset), 
    ...
    def save_current_frame(self, frame):
        ...
        filename = f"qr_capture_no_restrictions_{timestamp}.jpg"
    ...
    def run(self):
        """主运行循环"""
        print("Starting real-time QR code detection (no restrictions)...")
        ...
        try:
            for frame in self.camera.frames():
                ...
                # 显示帧
                cv2.imshow("QR Code Detection - No Restrictions", frame)
...
if __name__ == "__main__":
    # 创建并运行实时预览
    preview = QRCodeRealtimePreviewNoRestrictions()
    preview.run()